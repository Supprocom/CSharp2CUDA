    ...
    ImmutableArray<Diagnostic>.Builder diagnostics) : CSharpSyntaxWalker
{
    ...
    public override void VisitInvocationExpression(InvocationExpressionSyntax node)
    {
        ...
        else
        {
            ...
            else if (call.Kind is CudaCallKind.Atomic or CudaCallKind.SignedInt64Atomic)
            {
                ...
                else if (TryGetConstantArrayWriteLocation(
                    arguments[0].Expression,
                    out var constant))
                {
                    ReportConstantWrite(arguments[0].Expression, constant);
                }
            }
    ...
    private void ValidateBinary(BinaryExpressionSyntax node)
    {
        ...
        {
            if (operatorMethod.ContainingType.ToDisplayString() != "Supprocom.CSharp2CUDA.CudaInt32" ||
    ...
    private void ValidateIncrement(
        ...
        bool prefix)
    {
        if (operation.Target.Syntax is ExpressionSyntax target &&
            TryGetConstantArrayWriteLocation(target, out var constant))
        {
            ReportConstantWrite(target, constant);
            return;
        }
    ...
    private bool TryGetConstantArrayWriteLocation(
        ExpressionSyntax expression,
        out CudaConstantArrayPlan constant)
    {
        while (expression is ParenthesizedExpressionSyntax parenthesized)
            expression = parenthesized.Expression;

        if (expression is ElementAccessExpressionSyntax element &&
            semanticModel.GetSymbolInfo(element.Expression).Symbol is IFieldSymbol field &&
            plan.TryGetConstantArray(field, out constant!))
        {
            return true;
        }
    ...
    private static SpecialType EffectiveType(ITypeSymbol? type) =>
        type?.ToDisplayString() == "Supprocom.CSharp2CUDA.CudaInt32"
    ...
    private void ReportConstantWrite(SyntaxNode node, CudaConstantArrayPlan constant) =>
        diagnostics.Add(Diagnostic.Create(
